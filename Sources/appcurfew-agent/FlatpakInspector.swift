...
struct FlatpakInspector {
    ...
    func runningAppIdentifiers() throws -> Set<String> {
        let output = try runFlatpak(arguments: ["ps", "--columns=application"])
        
        let lines = output.split(separator: "\n").map(String.init)
            .filter { !$0.hasPrefix("Application") }   // ← changed from "Application ID"
